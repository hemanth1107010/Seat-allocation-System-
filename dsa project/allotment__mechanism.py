...
def quicksort(arr, column='MARKS', reverse=True):
    if len(arr) <= 1:
        return arr
    else:
        pivot = arr.sample(1).iloc[0][column]
        left = arr[arr[column] > pivot]
        middle = arr[arr[column] == pivot]
        right = arr[arr[column] < pivot]
        return quicksort(left, column, reverse) + list(middle) + quicksort(right, column, reverse)
    ...
    def run_allotment(self):
        ...
        if self.allotment_done == True:
            ...
            with use_scope("main", clear=True):
                ...
                data = input_group(
                    "Reset Allotment or return to menu", [actions('', [{'label': 'Back', 'value': 1},
                                                                       {'label': 'Reset', 'value': 2}], name='action',
                                                                   help_text=None), ])
                if data["action"] == 2:
            ...
            allotment = pd.DataFrame(quicksort(allotment_pre))
            allotment.columns = df.columns  # Maintain original column names
        ...
        with open("datasheet.csv", 'r') as f:
            ...
            for row in reader_object:
                if (row[0] == person[0]) and row[1] == person[1]:
                    # return the row no. where the record is found
